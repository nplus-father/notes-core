...
3. **導覽入口 `<GuideCard>`（v0.30.0）**：站上有 guide 內容（`src/content/guide/`）時出現——章節目錄＋「從第一章開始讀」，是總覽（三段摘要）與導覽（完整長文）之間的「往深處讀」動線。資料直接讀 guide collection，站台不接線。
4. **書架 `<Bookshelf>`**：本站彙整自哪些 owned books（見 §4）。**v0.29.0 起固定高度**（約兩排半封面）＋垂直捲動，標題列右端補書數——書多的站（startup 61 本）攤開就是五六排 190px 的封面。有 bibliography 的站，標題列再補「盤點與開採度 →」連 `/library/`。
...
- **單頁長文＋左側 sticky 章節目錄**，不做每章一頁——五章是一份導覽的分節，分頁會打斷「一口氣讀完」的敘事。章序用**襯線中文數字**（第一章…），accent 沿 `data-axis` 分岔（人物暖褐／主題冷藍），導覽天生帶站型識別。
