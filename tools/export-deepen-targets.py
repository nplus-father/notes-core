...
**2026-08-27 狀態註記：紅利已盡，本報告轉為增量監測。** 五輪開單實測的假陽性軌跡：
第五輪 12 查 5 開，第六輪 **19 查 0 開**——剩餘候選幾乎全是「**單頁已厚挖**」（本星系的
頁天生厚：5–12 條 core＋details＋案例，TOC 頁連鼓繩與三指標都收了）。試過用引用頁的
大小自動排除，失敗：task-separation 6.8KB 當初該開第二頁、exegetical-fallacies 6.8KB
不該開——**判準是「書的哪些面向被涵蓋」，不是頁多厚，本質不可自動化**。
結論：存量的深化債已實質清畢；本報告日後的用途是**新書進站後的增量監測**
（新收的大部頭掛上第一頁後會浮出來），不再是現成的待辦來源。
