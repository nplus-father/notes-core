#!/usr/bin/env bash
# 一次重算所有生成型星系盤點文件（docs/ 下的七份），並印出這一輪的落差。
#
# 用法：
#   tools/refresh-galaxy-docs.sh            # 重算七份，印摘要與 diffstat
...
# 只看 docs/ 底下這七份；其他 docs 是手維護的，不該被這支影響。
TRACKED=(WANTED-BOOKS.md ORPHAN-BOOKS.md DEEPEN-READY.md MISSING-YEARS.md YEAR-CONFLICTS.md DEEPEN-TARGETS.md ANCHOR-GAPS.md)
