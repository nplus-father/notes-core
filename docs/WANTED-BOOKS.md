...
> **生成於 2026-09-14T11:57:44+08:00**｜由 `tools/export-wanted.py` 產生，**不要手改**——改資料源再重跑。
...
**已收錄比對的資料源**：GitHub 現況（`gh repo list` nplus-father／Andrewnplus，1909 個 repo）。
...
| status | 意思 | 判準 | 筆數 |
| --- | --- | --- | --- |
| `owned` | **已收錄**——書已經做成 `nplus.wiki/<slug>/` 的書站 | 必須有 `slug`；首頁書架的封面列就是取這批，概念頁的 `furtherReading.anchor` 也是連到這裡 | 2278 筆（去重 1768 本） |
...
> `owned` 去重後的 1768 是**已建成書站的書**（2278 是含跨站重複的登錄筆數，
