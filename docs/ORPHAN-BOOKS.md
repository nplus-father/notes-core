...
> **生成於 2026-08-26T12:13:55+08:00**｜由 `tools/export-orphan-books.py` 產生，**不要手改**——改資料源再重跑。
...
| 檢查 | 數 | 後果 |
| --- | ---: | --- |
| 孤兒書（沒有任何站的 bibliography 指到） | **336** | 書站建了但沒有筆記在用，等於白建 |
...
| `commentary` | theology | 10/59 | 17% | biblical-studies(44)、stott(6) |
...
#### `growth` — 25/85 沒人認領（目前：growth(21)、tracy(10)、wan-weigang(8)）
...
#### `systematic` — 22/43 沒人認領（目前：theology(19)、peterson(1)、biblical-studies(1)）
...
| `christian-theology-reader` | The Christian Theology Reader | Alister McGrath |
...
| `is-hell-for-real` | Is Hell for Real | Christopher W. Morgan & Robert A. Peterson (eds.) |
...
#### `ethics` — 14/49 沒人認領（目前：philosophy(12)、fromm(10)、taleb(3)）
...
#### `commentary` — 10/59 沒人認領（目前：biblical-studies(44)、stott(6)）
...
| `life-in-three-dimensions` | 心理富足的人生：好奇、探索與體驗如何成就更完整的生活 | 大石繁宏 |
...
#### `personal-finance` — 9/55 沒人認領（目前：personal-finance(33)、kiyosaki(15)、tracy(3)）
...
| `wealthy-barber-returns` | The Wealthy Barber Returns | David Barr Chilton |
...
#### `marriage` — 2/11 沒人認領（目前：relationships(7)、life-meaning(1)、templar(1)）
...
#### `dating` — 2/7 沒人認領（目前：relationships(5)、cloud(1)）
...
#### `visual` — 1/14 沒人認領（目前：design(11)、problem-solving(2)、de-botton(1)）
...
#### `security` — 1/15 沒人認領（目前：security(14)、system-design(1)、cloud-infra(1)）
...
#### `sales` — 1/9 沒人認領（目前：business-strategy(5)、tracy(2)、kiyosaki(1)）
