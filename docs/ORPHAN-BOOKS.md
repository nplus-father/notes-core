...
> **生成於 2026-09-25T21:00:45+08:00**｜由 `tools/export-orphan-books.py` 產生，**不要手改**——改資料源再重跑。
...
**資料源**：GitHub 現況（`gh repo list` nplus-father／Andrewnplus，1946 個 repo），其中 `nplus-kind-book` 的書 repo 1808 本（9 本經 [EXCLUDED-BOOKS.md](./EXCLUDED-BOOKS.md) 裁決排除，不入盤點）。
